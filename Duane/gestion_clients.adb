...
PACKAGE BODY Gestion_Clients IS
   ...
   FUNCTION Inferieur (Id1, Id2 : T_Identite) RETURN Boolean IS
   ...
   BEGIN
      IF Arbre = NULL THEN
         ...
         Arbre := NEW T_Arbre_Client'(Val => Nouveau_Client, Fg => NULL, Fd => NULL);
  ...
  FUNCTION Rechercher_Client (
   ...
   BEGIN
      IF Tete = NULL THEN
         RETURN NULL;
      ELSE
         IF Client.Nom(1..Client.Knom) = Tete.Val.Id_Client.Nom(1..Tete.Val.Id_Client.Knom) AND THEN
            Client.Prenom(1..Client.Kprenom) = Tete.Val.Id_Client.Prenom(1..Tete.Val.Id_Client.Kprenom) THEN
            RETURN Tete;
            
         ELSIF Client.Nom(1..Client.Knom) < Tete.Val.Id_Client.Nom(1..Tete.Val.Id_Client.Knom) OR ELSE
              (Client.Nom(1..Client.Knom) = Tete.Val.Id_Client.Nom(1..Tete.Val.Id_Client.Knom) AND THEN
               Client.Prenom(1..Client.Kprenom) < Tete.Val.Id_Client.Prenom(1..Tete.Val.Id_Client.Kprenom)) THEN
   ...
   PROCEDURE Enregistrer_Reglement (
      ...
      Montant      : Integer;
   BEGIN
      Put_Line("---  reglement d'une afcture ---");
      ...
      ELSE
         Put_Line("Facture en instance : " & Integer'Image(Cible.Val.Facture) & " euros.");
            ...
            LOOP
               BEGIN
                  ...
                  ELSIF Montant > Cible.Val.Facture THEN
                     Put_Line("Erreur : Le montant ne peut pas depasser la facture.");
                  ELSE
                     Cible.Val.Facture := Cible.Val.Facture - Montant;
                     Cible.Val.Montant_Regle := Cible.Val.Montant_Regle + Montant;
