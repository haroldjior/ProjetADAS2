...
package body gestion_date is
   ...
   function date_anterieure (date1, date2 : in T_date) return boolean is
   begin
      if date1.annee < date2.annee then
         return (true);
      elsif date1.annee = date2.annee then
         if T_liste_mois'pos (date1.mois) < T_liste_mois'pos (date2.mois) then
            return (true);
         elsif T_liste_mois'pos (date1.mois) = T_liste_mois'pos (date2.mois)
         then
