with ada.text_io, ada.characters.handling;
use ada.text_io, ada.characters.handling;
with gestion_client;

package body gestion_identite is

   --Saisie du nom et prenom d'une personne, selon les contraintes du sujet
   procedure saisie_identite (id : out T_identite) is
      mot    : T_mot := (others => ' ');
      k      : Integer;
      erreur : Boolean;

      --Procedure auxilliaire qui verifie la validite du nom saisie
      procedure verif_nom (nom : in T_Nom; erreur : out boolean) is
         n_trait : integer := 0;
      begin
         erreur := false;

         --Saisie vide
         if nom.knom = 0 then
            erreur := true;
         end if;

         --Commence ou termine par _
         if (mot (k) = '_') or (mot (1) = '_') then
            erreur := true;
         end if;

         for i in 1 .. k loop

            --Contient un chiffre au milieu ou contient des caractères interdits
            if (mot (i) not in '0' .. '9' | 'a' .. 'z' | 'A' .. 'Z' | '_') then
               erreur := true;
               exit;
            elsif (mot (i) in '0' .. '9')
              and then (mot (i + 1) in 'a' .. 'z' | 'A' .. 'Z')
            then
               erreur := true;
               exit;

            --Conteur de _
            elsif mot (i) = '_' then
               n_trait := n_trait + 1;
            end if;
         end loop;

         --Contient plus de 1 _
         if n_trait > 1 then
            erreur := true;
         end if;
      end verif_nom;

   begin
      loop
         begin
            put ("Nom : ");
            get_line (mot, k);
            id.id_nom.nom := to_upper (mot);
            id.id_nom.knom := k;
            verif_nom (id.id_nom, erreur);
            exit when erreur = false;
            put_line ("/!\ Saisie invalide");
         exception
            when Constraint_Error =>
               put_line ("/!\ Saisie invalide");
         end;
      end loop;
      loop
         begin
            put ("Prenom : ");
            get_line (mot, k);
            id.id_prenom.nom := to_upper (mot);
            id.id_prenom.knom := k;
            verif_nom (id.id_prenom, erreur);
            exit when erreur = false;
            put_line ("/!\ Saisie invalide");
         exception
            when Constraint_Error =>
               put_line ("/!\ Saisie invalide");
         end;
      end loop;

   end saisie_identite;

   --Affichage de l'identite d'une personne
   procedure affichage_id (id : in T_identite) is
   begin
      put ("Nom : ");
      put (id.id_nom.nom (1 .. id.id_nom.knom));
      new_line;
      put ("Prenom : ");
      put (id.id_prenom.nom (1 .. id.id_prenom.knom));
      new_line;
   end affichage_id;

end gestion_identite;
